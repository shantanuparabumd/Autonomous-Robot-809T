...
class Robot:
    ...
    def create_map(self,blocks):
        for i in blocks:
            x,y=self.get_block_pose(i[0],math.radians(i[1]))
    ...
    def get_location(self,RP):
        ...
        DISTANCE=(WHEEL_REV*self.DIST_PER_REV)+WHEEL_REV*0.25
        self.get_yaw()
...
# self.L_IN,self.R_OUT,self.L_OUT,self.R_IN
    ...
    def move_to_pos(self,x,y):
        ...
        if not self.check_goal(x,y,X,Y):
            if angle<2+set_angle and angle>set_angle-2:
                self.control()
                self.MOVING=1
                X,Y,Theta,c_distance=self.get_location(self.pose)
    ...
    def main(self):
        ...
        for frame in self.camera.capture_continuous(self.rawCapture, format="bgr", use_video_port=False):
            ...
            # self.get_count()
            loc=[[0.25,0.25],[0.25,0.5],[0.5,0.5]]
            if self.MOVING==0:
                if self.POSITION>2:
                    exit(0)
                else:
                    ...
                    cv2.putText(img,f"X: {x} Y: {y}", (100,400), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
